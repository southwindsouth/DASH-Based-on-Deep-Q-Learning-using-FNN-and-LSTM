...
BATCH_SIZE = 100
ONE_BATCH = 10
NUM_OF_BATCH = int(BATCH_SIZE/ONE_BATCH)
num_features = 4
# - FFN
...
class ActionSelector(object):
    ...
    def action(self, state):
        if self.greedy_policy:
            with torch.no_grad():
                out_action, (hn, cn) = policy_net(state.tensor().to(device).reshape(1, 1, num_features))
...
def simulate_dash(sss, bws, memory, phase, batch_size):
    ...
    for i_episode in range(num_episodes):
        ...
        for t in range(CH_HISTORY, num_segments):
            ...
            new_state_batch = state_batch.reshape(NUM_OF_BATCH, ONE_BATCH, num_features)
            new_next_state_batch = next_state_batch.reshape(NUM_OF_BATCH, ONE_BATCH, num_features)
            ...
            out_of_hidden1 = forwardCalculation(output1).reshape(BATCH_SIZE, num_features)
            state_action_values = out_of_hidden1.gather(1, action_batch.view(BATCH_SIZE, -1)).double()
            ...
            out_of_hidden2 = forwardCalculation(output2).reshape(BATCH_SIZE, num_features)
...
if __name__ == "__main__":
    ...
    np.savez('dash_LSTM_without_target_300', data_sqs=mean_sqs, data_rewards=mean_rewards)
